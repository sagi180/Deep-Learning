...
print(colorize.blue '==>' ..' Start to create configuritions model ##################')
for i1 = 1, #batchSizeArray do
	for i2 = 1, #epochsArray do 
		for i3 = 1, #drop do
			for i4 = 1, #drop do
				for i5 = 1, #hFlipList do
					for i6 = 1, #elseList do
						for i7 = 1, #elseList do
							batchSize = batchSizeArray[i1]
							epoch = epochsArray[i2]
							a = hFlipList[i5]
							b = elseList[i6]
							c = elseList[i6]
							d = elseList[i7]
							...
							drop1 = drop[i3]
							drop2 = drop[i4]
							name = 'log_adagrad_batch' .. batchSize .. '_drop1' .. drop1 .. '_drop2' .. drop2 .. '_a' .. a .. '_b' .. b .. '_c' .. c .. '_d' .. d .. '_e' .. e .. '_f' .. f
...
print(colorize.blue '==>' ..' End to create configuritions model, configuritions numbers is =  ' .. #listOfConfiguration)
...
function trainAndEval(model, trainData, trainLabels, testData, testLabels, epochs, optimFunc, batchSize , optimState, configName)
	...
	for e = 1, epochs do
		...
		if (e - maxTestAccEpoch) > 400 then
			print('We received that it\'s been a long time since the last time that epoch improved the accuracy (400 epochs pass), so I finish the run. last epoch is = ' .. e)
			break
		end
		--if e % 2 == 0 then
...
for i = 1, #listOfConfiguration do
	runModel(listOfConfiguration[i])
end 